...
const ChitarraTrainer = () => {
  ...
  const [customPattern, setCustomPattern] = useState([
    ...
    { direction: 'SU', duration: 0.5, beat: '&' }, // Nota: qui puoi mettere '+' o 'e' se preferisci
  ...
  // Refs per accesso sincrono (evitano i bug audio)
  ...
  // Sincronizzazione dei dati per il timer
  useEffect(() => {
  ...
  const playChord = (chordName, isDown) => {
    ...
    // Fallback sicuro per non crashare se l'accordo è scritto male
    ...
    frequencies.forEach((freq, index) => {
      ...
      oscillator.type = 'triangle'; // Suono un po' più "pieno" della sine
  ...
  // --- LOOP PRINCIPALE (Logica corretta con Refs) ---
  useEffect(() => {
    if (!isPlaying) {
    ...
    function runMainLoop() {
        const scheduleNext = () => {
            ...
            // 2. CALCOLO ACCORDO (Prima di suonare!)
  ...
  // Funzione per la Modulazione Velocità
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4 flex items-center justify-center">
      {/* Stile CSS per nascondere le scrollbar */}
      <style>{`
      ...
      <div className="max-w-4xl w-full bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-8">
        ...
        {showSettings && (
          <div className="mb-8 bg-white/10 rounded-xl p-6 max-h-96 overflow-y-auto scrollbar-hide">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="text-white/70 text-sm block mb-2">Progressione Accordi (separati da spazio)</label>
                <input
                  type="text"
                  value={chordsInput}
                  onChange={(e) => {
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-white/70 text-sm">Pattern Ritmo Personalizzato</label>
                ...
                <div className="space-y-2">
                  {customPattern.map((step, index) => (
                    <div key={index} className="bg-white/5 rounded-lg p-3 flex items-center gap-2">
                      ...
                      <select value={step.direction} onChange={(e) => updatePatternStep(index, 'direction', e.target.value)} className="bg-white/10 text-white rounded px-2 py-1 text-sm border border-white/20 focus:outline-none">
                        <option value="GIÙ">↓ GIÙ</option>
                        <option value="SU">↑ SU</option>
                      </select>
                      <select value={step.duration} onChange={(e) => updatePatternStep(index, 'duration', parseFloat(e.target.value))} className="bg-white/10 text-white rounded px-2 py-1 text-sm border border-white/20 focus:outline-none">
                      ...
                      {customPattern.length > 1 && (
                        <button onClick={() => removePatternStep(index)} className="bg-red-500/20 hover:bg-red-500/40 text-red-300 rounded p-1"><Trash2 size={16} /></button>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="mb-8 flex items-center justify-center gap-4">
            ...
            <div className="inline-block bg-white/20 rounded-2xl px-8 py-4 shadow-lg">
              ...
              <div className="text-3xl font-bold text-white/80 text-center">
                {rawSong[nextChordDisplayIndex] ? rawSong[nextChordDisplayIndex].chord : '-'}
              </div>
            </div>
          </div>
        )}

        {/* Pattern Visivo (con fix scrollbar-hide) */}
        {(!isPlaying || countInDone) && (
              ...
              {rawPattern.map((move, index) => {
                ...
                return (
                  <div key={index} className={`relative rounded-2xl p-6 transition-all duration-150 flex-shrink-0 w-24 ${isActive ? 'bg-gradient-to-br from-green-400 to-emerald-500 scale-110 shadow-2xl shadow-green-500/50' : isPast ? 'bg-white/20' : 'bg-white/10'} ${move.isFirst ? 'ring-4 ring-yellow-400' : ''}`}>
                    {move.isFirst && <div className="absolute -top-3 -right-3 bg-yellow-400 text-black text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center z-10">1</div>}
                    <div className="absolute -top-3 -left-3 bg-white/30 text-white text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center z-10">{move.beat}</div>
        ...
        {/* Controlli & Modulazione Velocità */}
        <div className="space-y-6">
          ...
          <div className="bg-white/5 rounded-xl p-6">
            ...
            <div className="flex items-center justify-center gap-4">
              <button
                onClick={() => adjustBpm(-5)}
                className="bg-white/10 hover:bg-white/20 text-white rounded-lg px-4 py-2 font-bold transition-all"
              >
                -5
              </button>
              <div className="text-4xl font-black text-white w-24 text-center">
                {bpm}
              </div>
              <button
                onClick={() => adjustBpm(5)}
        ...
        <div className="mt-8 bg-white/5 rounded-xl p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-white/80">
            <div className="flex items-center gap-2">
              ...
              <span>Plettrata attiva</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full ring-2 ring-yellow-400"></div>
              <span>Primo battere (accento)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full bg-white/20"></div>
              <span>Plettrata passata</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="text-white/70 font-bold">⚙️</div>
              <span>Personalizza ritmo e accordi</span>
            </div>
          </div>
        </div>
