...
const ChitarraTrainer = () => {
  ...
  const [chordsInput, setChordsInput] = useState(
    'SOL SIm DO RE SOL DO SOL RE DO SOL'
  );
  ...
  // --- CALCOLO DATI ---
  const rawSong = chordsInput
    .trim()
    .split(/\s+/)
    .map((chord) => ({ chord, bars: 1 }));
  ...
  const noteFrequencies = {
    DO: [130.81, 164.81, 196.0, 261.63, 329.63, 523.25],
    DO7: [130.81, 164.81, 196.0, 233.08, 261.63, 329.63],
    DOm: [130.81, 155.56, 196.0, 261.63, 311.13, 392.0],
    RE: [146.83, 220.0, 293.66, 146.83, 220.0, 369.99],
    REm: [146.83, 220.0, 293.66, 146.83, 174.61, 220.0],
    RE7: [146.83, 220.0, 261.63, 146.83, 220.0, 293.66],
    MI: [164.81, 246.94, 329.63, 164.81, 207.65, 329.63],
    MIm: [164.81, 246.94, 329.63, 164.81, 196.0, 246.94],
    MI7: [164.81, 246.94, 293.66, 164.81, 196.0, 246.94],
    FA: [87.31, 174.61, 220.0, 261.63, 349.23, 174.61],
    FAm: [87.31, 174.61, 207.65, 261.63, 349.23, 174.61],
    'FA#m': [92.5, 185.0, 220.0, 277.18, 369.99, 185.0],
    'FA#m7': [92.5, 185.0, 220.0, 164.81, 277.18, 185.0],
    SOL: [196.0, 246.94, 293.66, 392.0, 493.88, 196.0],
    SOLm: [196.0, 233.08, 293.66, 392.0, 466.16, 196.0],
    'SOL#m': [207.65, 246.94, 311.13, 415.3, 493.88, 207.65],
    SOL7: [196.0, 246.94, 293.66, 174.61, 392.0, 196.0],
    LA: [110.0, 220.0, 277.18, 329.63, 440.0, 220.0],
    LAm: [110.0, 220.0, 261.63, 329.63, 440.0, 220.0],
    LA7: [110.0, 220.0, 277.18, 196.0, 329.63, 220.0],
    ...
    function runMainLoop() {
      const scheduleNext = () => {
        ...
        if (liveSong[activeChordIndex]) {
          playChord(
            liveSong[activeChordIndex].chord,
            currentPatternStep.isDown
          );
        }
  ...
  const addPatternStep = () =>
    setCustomPattern([
      ...customPattern,
      { direction: 'GIÙ', duration: 0.5, beat: '?' },
    ]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 p-4 flex items-center justify-center">
      {/* CSS per nascondere le scrollbar */}
      <style>{`
      ...
      <div className="max-w-4xl w-full bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/10 text-white rounded-lg px-4 py-2 border border-white/20 focus:border-white/50 focus:outline-none"
                />
              </div>

              <div>
                <label className="text-white/70 text-sm block mb-2">
                  Progressione Accordi (separati da spazio)
                </label>
                <input
                  type="text"
                  value={chordsInput}
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-white/70 text-sm">
                    Pattern Ritmo Personalizzato
                  </label>
                  <button
                    onClick={addPatternStep}
                    className="bg-green-500 hover:bg-green-600 text-white rounded-lg px-3 py-1 text-xs flex items-center gap-1"
                  >
                ...
                <div className="space-y-2">
                  {customPattern.map((step, index) => (
                    ...
                    >
                      <div className="text-white/70 text-sm w-8">
                        {index + 1}.
                      </div>
                      <select
                        value={step.direction}
                        onChange={(e) =>
                          updatePatternStep(index, 'direction', e.target.value)
                        }
                        className="bg-white/10 text-white rounded px-2 py-1 text-sm border border-white/20 focus:outline-none"
                      >
                        <option value="GIÙ">↓ GIÙ</option>
                      ...
                      <input
                        ...
                        value={step.beat}
                        onChange={(e) =>
                          updatePatternStep(index, 'beat', e.target.value)
                        }
                        className="bg-white/10 text-white rounded px-2 py-1 text-sm w-12 border border-white/20 focus:outline-none text-center"
                      />
                      {customPattern.length > 1 && (
                        <button
                          onClick={() => removePatternStep(index)}
                          className="bg-red-500/20 hover:bg-red-500/40 text-red-300 rounded p-1"
                        >
                          <Trash2 size={16} />
                        </button>
                      )}
          ...
          <div className="mb-8 flex items-center justify-center gap-4">
            <div className="inline-block bg-gradient-to-r from-yellow-400 to-orange-500 rounded-2xl px-12 py-6 shadow-2xl transform transition-all duration-300">
              ...
              <div className="text-6xl font-black text-white text-center">
                {rawSong[currentChord] ? rawSong[currentChord].chord : '-'}
              </div>
            </div>
            ...
            <div className="inline-block bg-white/20 rounded-2xl px-8 py-4 shadow-lg">
              ...
              <div className="text-3xl font-bold text-white/80 text-center">
                {rawSong[nextChordDisplayIndex]
                  ? rawSong[nextChordDisplayIndex].chord
                  : '-'}
              </div>
            </div>
          </div>
        ...
        {/* Pattern Visivo (con fix scrollbar-hide e padding extra) */}
        {(!isPlaying || countInDone) && (
            ...
            <div className="flex gap-4 justify-center min-w-max mx-auto w-fit p-4">
              {rawPattern.map((move, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`relative rounded-2xl p-6 transition-all duration-150 flex-shrink-0 w-24 ${
                      isActive
                        ? 'bg-gradient-to-br from-green-400 to-emerald-500 scale-110 shadow-2xl shadow-green-500/50'
                        : isPast
                        ? 'bg-white/20'
                        : 'bg-white/10'
                    } ${move.isFirst ? 'ring-4 ring-yellow-400' : ''}`}
                  >
                    {move.isFirst && (
                      <div className="absolute -top-3 -right-3 bg-yellow-400 text-black text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center z-10">
                        1
                      </div>
                    )}
                    <div className="absolute -top-3 -left-3 bg-white/30 text-white text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center z-10">
                      {move.beat}
                    </div>
        ...
        {/* Progressione (con fix scrollbar-hide) */}
            ...
            {rawSong.map((item, index) => (
              <div
                ...
                className={`px-4 py-2 rounded-lg font-bold transition-all duration-300 ${
                  currentChord === index
                    ? 'bg-gradient-to-r from-yellow-400 to-orange-500 text-white scale-110 shadow-lg z-10'
                    : index === nextChordDisplayIndex
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          <button
            onClick={togglePlay}
            className={`flex items-center gap-2 px-8 py-4 rounded-xl font-bold text-lg transition-all transform hover:scale-105 shadow-xl ${
              isPlaying
          ...
          <button
            onClick={reset}
            className="flex items-center gap-2 px-8 py-4 rounded-xl font-bold text-lg bg-white/10 hover:bg-white/20 text-white transition-all transform hover:scale-105 shadow-xl"
          >
            <RotateCcw size={24} /> RESET
          </button>
          <button
            onClick={() => setSoundEnabled(!soundEnabled)}
            className={`flex items-center gap-2 px-6 py-4 rounded-xl font-bold text-sm transition-all transform hover:scale-105 shadow-xl ${
              soundEnabled
          ...
          <button
            onClick={() => setChordSoundEnabled(!chordSoundEnabled)}
            className={`flex items-center gap-2 px-6 py-4 rounded-xl font-bold text-sm transition-all transform hover:scale-105 shadow-xl ${
              chordSoundEnabled
